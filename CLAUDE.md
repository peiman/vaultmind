# Claude Code Guidelines for VaultMind (built on the ckeletin-go scaffold)

> **Before anything else:** VaultMind can maintain an identity vault (conventionally `vaultmind-identity/`) wired via `vaultmind hooks install` — see the README integration section. No identity vault ships in this repo.
...
## VaultMind — try it on the example vault

This repo ships a small **fictional** example vault at `examples/ada-vault/` (the agent *Ada*). Use it to see VaultMind's own retrieval working — and to dogfood the tool you're changing:

```bash
vaultmind index --vault examples/ada-vault && vaultmind index --embed --vault examples/ada-vault
vaultmind ask "who are you" --vault examples/ada-vault --json
vaultmind search "scope" --vault examples/ada-vault --json
vaultmind note get identity-who-am-i --vault examples/ada-vault --json
vaultmind doctor --vault examples/ada-vault
```
