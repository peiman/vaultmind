...
func TestRebuild_CompletesInReasonableTime(t *testing.T) {
	...
	// ~8ms/note; 200ms/note leaves ample headroom. Skipped under -race: the
	// detector's instrumentation adds heavy, variable overhead that makes any
	// wall-clock budget meaningless (a 7-note rebuild measured 1.46s vs the
	// 1.4s budget on CI). The correctness assertions above still run.
	if !raceEnabled {
