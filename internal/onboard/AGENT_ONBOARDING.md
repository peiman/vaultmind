...
If no binary is reachable, the user can install one without cloning: `go install github.com/peiman/vaultmind@latest` (pure-Go MiniLM, any platform), or — for the full BGE-M3 hybrid on darwin-arm64/linux-amd64 — download the prebuilt ORT archive (`vaultmind_<version>_<os>_<arch>_ort.tar.gz`) from the GitHub release, extract, and use that path. (Cloning + `task build` is the from-source alternative.) Tell the user the options and ask for the resulting path. Stop until you have one.
...
> I see this is a [Go CLI / Python lib / TypeScript app / mixed-language monorepo / etc.]. From your CLAUDE.md, you work on [whatever you read]. Here's what vaultmind will do for this project specifically:
>
...
Show the user the output. Confirm it returns their identity note. If `top_hit_confidence` is `no_match`, the identity note may be too short to embed well — interview them for one more sentence and re-index. **But do NOT re-interview on `weak` alone:** on a small/tight persona vault even a *correct* identity hit legitimately reads `weak` — the vault's notes are all about one person, so the embedder can't spread them far above the noise floor (`ask` prints a one-line "tight vault" contrast hint when this happens). Check `--explain` and the top-hit *id*; only act if the top hit is the wrong note.
...
**Branch A — non-empty vocabulary** (the common-rich case, e.g. a migrating repo uses `contract, plan, log, research, process, spec, manifesto, architecture`):
...
**Branch B — empty vocabulary** (no `type:` field anywhere, common case in a markdown-as-prose corpus):
...
validation. Per the extend-don't-overwrite principle, vaultmind never
...
Claude Code reads hooks from exactly two project-level files: `.claude/settings.json`
(committed, team-shared) and `.claude/settings.local.json` (gitignored, personal).
**`.claude/hooks.json` is NOT an official hook-config location** — a standalone
project `hooks.json` is non-standard and was broken in Claude Code 2.1.129; only a
plugin's bundled `hooks/hooks.json` is read. Check both official files, and look for
a stray `hooks.json` to flag (do not write to it):

```bash
ls -la .claude/ 2>/dev/null
cat .claude/settings.json .claude/settings.local.json 2>/dev/null
test -f .claude/hooks.json && echo "NOTE: .claude/hooks.json exists — non-standard; its hooks may not fire. Do NOT add ours here."
```

**Where to write our 5 hook entries depends on what the vault IS:**
- **Personal persona** (user-level vault, e.g. `~/.vaultmind/persona`, machine-specific
  path) → `.claude/settings.local.json` (gitignored). Committing it would break
  teammates who lack that vault. This is the default and what vaultmind's own repo does.
- **Project-shared agent memory** (an in-repo vault, e.g. `<repo>/vaultmind-identity/`) →
  `.claude/settings.json` (committed) so the whole team gets it.

Three modes:

- **Fresh** (no `.claude/` at all): create the chosen file and write our hook entries.
- **Coexisting** (a `.claude/` exists, with hooks in `settings.json` and/or
  `settings.local.json`): **MERGE our 5 entries into the existing `hooks.<Event>` array
  — never replace.** Hooks merge at the event level (all run), so a project's own
  `SessionStart` (e.g. `install_tools.sh`) and ours both fire; our `PreToolUse` matches
  `Read`, coexisting with a project's `PreToolUse·Bash`. Preserve every existing entry.
- **Conflict** (a hook script with our name already exists, or a settings file is
  malformed JSON): REFUSE, surface the conflict, propose a side-path.
...
**Install scripts AND wire settings in one command — `vaultmind hooks install --merge`:**

The canonical hook scripts are embedded inside the `vaultmind` binary (SSOT — they cannot drift from the source). `--merge` writes them AND merges the five hook entries into the project's settings file for you, so you never hand-edit JSON:

```bash
# 1. PREVIEW (writes nothing) — this IS your diff-before-write (see §6c)
vaultmind hooks install <project-dir> --vault <vault-path> --merge --dry-run

# 2. APPLY — writes scripts + merges the wiring into .claude/settings.json
vaultmind hooks install <project-dir> --vault <vault-path> --merge

#    --local   → wire .claude/settings.local.json (gitignored, personal) instead
#    --json    → machine-readable envelope
#    --force   → overwrite drifted script copies
```

**The merge NEVER clobbers.** It is additive and idempotent: a project's own hooks are preserved (its `SessionStart` and ours both fire; our `PreToolUse·Read` coexists with a project's `PreToolUse·Bash`), our entries are de-duplicated so re-running changes nothing, all other settings and key order are kept, and **malformed settings error out before any write** — your file is never corrupted. This replaces the hand-merge in §6a's Coexisting mode; you no longer transcribe the stanza above by hand (it's shown so you understand what `--merge` does).

`--vault` bakes `VAULTMIND_VAULT=<path>` into every wired command, so the hooks target the user's vault with **no script editing** — the path-template table below is only needed for hybrid (research-vault) setups or `capture-episode.sh`'s output dir.

The command writes **9 scripts** to `<project-dir>/.claude/scripts/`; **5 are wired as the core hooks**: `load-persona.sh` (SessionStart), `vaultmind-health.sh` (SessionStart — the vault-agnostic onboarding nudge that names the next command for the current install/index state), `vault-recall.sh` (UserPromptSubmit), `vault-track-read.sh` (PreToolUse·Read), `capture-episode.sh` (SessionEnd). The other 4 — `vault-block-read.sh`, `auto-rag-evaluate.sh`, `auto-rag-guard.sh`, `shell-strip.sh` — are helpers the core scripts source or that you opt into; written but not wired. Script writes refuse to overwrite drifted user-edited copies unless `--force` (see §6a Coexisting / Conflict modes). Omit `--merge` to only write scripts and print the stanza for manual merging (the pre-`--merge` behavior).

**Path-template work** (hybrid / custom output dir only) — the embedded scripts are project-portable: they use `$CLAUDE_PROJECT_DIR`, honor `VAULTMIND_VAULT` (set by `--vault`), and default to `vaultmind-identity/` / `vaultmind-vault/`. Edit a generated copy only for the cases below:
...
Show every diff. Get yes per file. Then write. With `--merge`, run it once
with `--dry-run` first — that printed preview IS the diff; show it, get the
user's yes, then re-run without `--dry-run` to apply.
...
To enable, add **two** PreToolUse entries to `.claude/settings.json` (Claude Code 2.1.129 needs `Bash` separate from `Write|Edit`; the pipe-alternation works within Write/Edit but not across all three):
...
The non-auto-RAG hooks installed by `vaultmind hooks install` also accept env-var overrides for projects with non-default vault names:
...
| `load-persona.sh` (SessionStart) | `VAULTMIND_PERSONA_MODE` | `served` | How the identity arrives. `served` hands it over as text; `explore` names the vaults and asks the agent to look before answering, serving nothing; `alternate` splits sessions between the two by session id (stable across compactions) so they can be compared. The arm is logged as `persona_mode` in `~/.vaultmind/persona-eval/`. |
...
If the user's vault isn't named `vaultmind-identity` (a project might use `companion-vault`; another might use `vaultmind-knowledge`), set the env var **inline in the settings.json command string** — bash interprets the assignment before invoking the script. No wrapper file needed for simple-value overrides like vault path or `AUTORAG_ALLOWED_ROOTS`. (Custom `DRIFT_CATALOG` JSON requires a wrapper — see below.)
...
-    {"matcher":"Bash","hooks":[{"type":"command","command":"bash \"$CLAUDE_PROJECT_DIR\"/.claude/scripts/auto-rag-guard.sh"}]},
-    {"matcher":"Write|Edit","hooks":[{"type":"command","command":"bash \"$CLAUDE_PROJECT_DIR\"/.claude/scripts/auto-rag-guard.sh"}]}
+    {"matcher":"Bash","hooks":[{"type":"command","command":"AUTORAG_VAULT=\"$CLAUDE_PROJECT_DIR\"/companion-vault bash \"$CLAUDE_PROJECT_DIR\"/.claude/scripts/auto-rag-guard.sh"}]},
+    {"matcher":"Write|Edit","hooks":[{"type":"command","command":"AUTORAG_VAULT=\"$CLAUDE_PROJECT_DIR\"/companion-vault bash \"$CLAUDE_PROJECT_DIR\"/.claude/scripts/auto-rag-guard.sh"}]}
   ]
```
...
```bash
#!/bin/bash
export AUTORAG_VAULT="$CLAUDE_PROJECT_DIR/companion-vault"
export DRIFT_CATALOG='[
  {"name":"force-push-main","tool":"Bash","match":"git\\s+push\\s+--force","decision":"deny","query":"never force-push main"}
]'
...
**Already-modified `auto-rag-guard.sh`?** A project running `vaultmind hooks install --force` will overwrite a hand-edited copy wholesale. Before `--force`:
...
- [ ] If Claude Code wired: the hook config (`.claude/settings.json` **or** `.claude/settings.local.json`, whichever you chose) has the 5 core hook entries; `.claude/scripts/` has all 9 scripts (5 wired + 4 helpers); restarting Claude Code surfaces an `IDENTITY CONTEXT` system-reminder.
...
**ORT install fails (libonnxruntime missing).** On darwin-arm64/linux-amd64 the simplest fix is to NOT build ORT yourself — download the prebuilt `vaultmind_<version>_<os>_<arch>_ort.tar.gz` from the GitHub release; it bundles libonnxruntime next to the binary (auto-found), full hybrid, no build. Otherwise tell the user how to install for a source build: `brew install onnxruntime` on macOS, then `task setup:ort` and re-build. Until that's done, vaultmind operates on the pure-Go path with MiniLM-only embeddings — usable but degraded retrieval quality. Migration / scaffolding still works.

**`.claude/settings.json` is malformed.** `hooks install --merge` detects this and **errors before any write** — it never corrupts the file — so it's safe to attempt. Don't try to fix the JSON yourself: tell the user *"Your settings.json doesn't parse as JSON; the merge refused to touch it. Point me at the syntax error and decide whether to fix it before continuing."* Once they fix it, re-run §6.
...
**The user is using Codex CLI.** Run §6 with `--agent codex`: `vaultmind hooks install <project-dir> --agent codex --vault <vault-path> --merge`. It writes `.codex/hooks.json` (identity at session start, health, per-prompt recall, decision-time context, episode capture at session end). Then tell the user, plainly, the steps Codex requires and skips SILENTLY without: *"Trust this project when Codex asks, then run `/hooks` inside Codex and trust the VaultMind hooks. Until both are done Codex starts with no memory and won't say so. After every VaultMind upgrade, run `/hooks` again: a new or changed hook is skipped until you approve it."* Do not take their word or yours for it — run `vaultmind hooks status <project-dir>`, which reads Codex's own record of approvals, names each VaultMind hook still unapproved, and fails until none are.
...
- For the architectural design, see the plasticity roadmap in the project README ("Status" section) — six steps, each a platform for the next.
...
- **Per-vault adapter for other non-Claude-Code agents** (Cursor, Aider, etc.) — Codex is wired (`--agent codex`, episode capture included); the rest are still out of scope.

## Source

- Companion principle: the lighter-move discipline — the discipline this doc honors. v1 covers greenfield + migration; hybrid and adapter-for-other-agents are deferred until reality demands.
- First real external user — this doc is the script they'll be onboarded with. Capture what breaks; it informs v2.
