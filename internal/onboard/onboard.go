...
//go:embed MESH_QUICKSTART.md
var meshQuickstart []byte
...
// MeshQuickStart returns the embedded mesh onboarding quick-start
// (MESH_QUICKSTART.md) as raw bytes. Same no-mutate contract as
// QuickStart: it is the "how the identity verbs fit together" narrative
// for joining a Contract-B agent network (admin + member journeys).
// Production callers should prefer PrintMeshQuickStart.
func MeshQuickStart() []byte {
	return meshQuickstart
}

// PrintMeshQuickStart writes the embedded mesh onboarding quick-start
// verbatim to the supplied writer. Used by
// `vaultmind identity --print-instructions`. Returns the writer's error
// if any; the embed itself cannot fail at runtime.
func PrintMeshQuickStart(w io.Writer) error {
	if _, err := w.Write(meshQuickstart); err != nil {
		return fmt.Errorf("writing mesh onboarding quick-start: %w", err)
	}
	return nil
}
