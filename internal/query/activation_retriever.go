...
// now returns the reference time for scoring (injected clock or wall clock).
func (r *ActivationRetriever) now() time.Time {
	if r.Now != nil {
		return r.Now()
	}
	return time.Now().UTC()
...
func (r *ActivationRetriever) Search(_ context.Context, _ string, limit, offset int, filters index.SearchFilters) ([]retrieval.ScoredResult, int, error) {
	...
	scores, _, err := experiment.ComputeBatchScoresAt(r.ExpDB, candidateIDs, r.Params, nil, r.now())
	if err != nil {
