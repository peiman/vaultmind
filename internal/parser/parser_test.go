...
func TestParse_RealVaultConceptNote(t *testing.T) {
	content, err := os.ReadFile(filepath.Join(testvault.FixtureVault(), "concepts", "act-r.md"))
	require.NoError(t, err)
	...
	// Wikilinks use Obsidian-compatible format: [[Target]] or [[Target|Display]].
	// Parser extracts the target (the text before any |).
...
func TestParse_RealVaultProjectNote(t *testing.T) {
	content, err := os.ReadFile(filepath.Join(testvault.FixtureVault(), "projects", "proj-vaultmind.md"))
	require.NoError(t, err)
	...
	assert.True(t, note.IsDomain)
	assert.Equal(t, "proj-vaultmind", note.ID)
	assert.Equal(t, "project", note.NoteType)
	assert.NotEmpty(t, note.FTSBody)
}

func TestParse_RealVaultDecisionNote(t *testing.T) {
	content, err := os.ReadFile(filepath.Join(testvault.FixtureVault(), "decisions", "decision-example.md"))
	require.NoError(t, err)
	...
	assert.True(t, note.IsDomain)
	assert.Equal(t, "decision-example", note.ID)
...
func TestParse_RealVaultAllNotes(t *testing.T) {
	vaultRoot := testvault.FixtureVault()
