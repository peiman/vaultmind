...
func TestUserSessionID_FirstSessionGetsNewID(t *testing.T) {
	...
	id, err := db.StartSessionWithCaller("/vault", "cli",
		map[string]any{"user": "user", "host": "laptop"})
...
func TestUserSessionID_SubsequentSessionWithinThresholdReuses(t *testing.T) {
	...
	db := openTestDB(t)
	meta := map[string]any{"user": "user", "host": "laptop"}
...
func TestUserSessionID_DifferentCallerGetsOwnSession(t *testing.T) {
	// The companion project's automated persona load and a manual cli query are
	...
	require.NoError(t, err)
	hookID, err := db.StartSessionWithCaller("/vault", "companion-persona-hook", meta)
	require.NoError(t, err)
...
func TestUserSessionID_DifferentUserGetsOwnSession(t *testing.T) {
	// one operator on their laptop and another on theirs — different operators,
	...
	s, err := db.StartSessionWithCaller("/vault", "cli",
		map[string]any{"user": "someuser", "host": "laptop"})
	require.NoError(t, err)
...
func TestUserSessionID_StaleSessionGetsNewID(t *testing.T) {
	...
	_, err := db.Exec(
		...
		`{"user": "user","host":"laptop"}`, "old-user-session")
	require.NoError(t, err)
