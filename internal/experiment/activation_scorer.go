...
func ComputeBatchScores(db *DB, noteIDs []string, params ActivationParams, similarities map[string]float64) (map[string]float64, map[string]map[string]float64, error) {
	return ComputeBatchScoresAt(db, noteIDs, params, similarities, time.Now().UTC())
}

// ComputeBatchScoresAt is ComputeBatchScores with an explicit reference time.
// Production passes time.Now(); tests pass a fixed instant so activation
// ranking is deterministic instead of depending on sub-millisecond wall-clock
// differences between the access events and the scoring call.
