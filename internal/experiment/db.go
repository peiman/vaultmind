...
FROM sessions;`,
	...
	// identifying the invoking agent (e.g. "companion-persona-hook",
...
CREATE INDEX IF NOT EXISTS idx_sessions_user_session ON sessions(user_session_id);`,
	// Version 6: calibration_snapshots — per-vault retrieval calibration measured
	// at index --embed time. noise_floor (N) is the cosine an off-topic probe
	// query gets to any note (the relevance floor); ntn_cosine_mu/sigma describe
	// the vault's embedding-space dispersion. All fields are content-free scalars
	// — no note ids, no content, no query text — so the row doubles as the
	// vault_features the federated study (Paper #2) aggregates. History is kept
	// (one row per measurement); LatestCalibration reads the most recent.
    ...
    noise_floor_probes INTEGER NOT NULL,
    probe_set_version  INTEGER NOT NULL,
    ntn_cosine_mu      REAL NOT NULL,
    ntn_cosine_sigma   REAL NOT NULL,
    ntn_sample_count   INTEGER NOT NULL
);`,
	// Version 7: vault_path discriminator on calibration_snapshots. The DB is
	// global (one experiments.db per machine, many vaults), so without a
	// discriminator the ask path's per-vault calibration lookup would return
	// whichever vault was calibrated last — reading vault A's noise floor for a
	// query against vault B. vault_path scopes the lookup. Nullable: SQLite can't
	// add a NOT NULL column without a default to an existing table, and pre-v7
	// rows have no known vault — they stay NULL and are never matched by the
	// scoped lookup, so callers fall back to the embedder default. vault_path is
	// a storage discriminator only; it is NEVER serialized into the federated
	// export (CalibrationSnapshot.VaultPath is `json:"-"`).
	`ALTER TABLE calibration_snapshots ADD COLUMN vault_path TEXT;
CREATE INDEX IF NOT EXISTS idx_calibration_vault ON calibration_snapshots(vault_path, created_at);`,
}
