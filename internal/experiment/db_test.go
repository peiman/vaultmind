...
func TestOpen_AppliesSchema(t *testing.T) {
	...
	expectedTables := []string{"sessions", "events", "outcomes", "calibration_snapshots"}
...
// Version 7 adds a vault_path discriminator + index to calibration_snapshots so
// the ask path can read THIS vault's calibration instead of the globally-most-
// recent one (a multi-vault correctness fix).
func TestOpen_CalibrationVaultScoped(t *testing.T) {
	db := openTestExpDB(t)

	var col string
	err := db.QueryRow(
		"SELECT name FROM pragma_table_info('calibration_snapshots') WHERE name='vault_path'",
	).Scan(&col)
	require.NoError(t, err, "calibration_snapshots must have a vault_path column")
	assert.Equal(t, "vault_path", col)

	var idx string
	err = db.QueryRow(
		"SELECT name FROM sqlite_master WHERE type='index' AND name=?", "idx_calibration_vault",
	).Scan(&idx)
	require.NoError(t, err, "idx_calibration_vault must exist")
	assert.Equal(t, "idx_calibration_vault", idx)
}
