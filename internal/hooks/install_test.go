...
func TestInstall_FreshDir_WritesAllCanonicalScripts(t *testing.T) {
	...
	for _, name := range res.Written {
		...
		// gates — the companion project 2026-05-07 CRITICAL caught this).
...
// TestInstall_PopulatesSettingsStanza — every successful install emits the
// .claude/settings.json wiring stanza so the operator never hand-transcribes
// it from the docs (issue #41). With a VaultPath, the stanza bakes
// VAULTMIND_VAULT into the commands so the hooks target that vault.
func TestInstall_PopulatesSettingsStanza(t *testing.T) {
	dir := t.TempDir()
	res, err := hooks.Install(hooks.InstallConfig{ProjectDir: dir})
	require.NoError(t, err)
	assert.NotEmpty(t, res.SettingsStanza, "install must emit the settings.json wiring stanza")
	assert.Contains(t, res.SettingsStanza, "vault-recall.sh")
	assert.NotContains(t, res.SettingsStanza, "VAULTMIND_VAULT=",
		"no --vault: stanza stays unparameterized (scripts use the default)")

	withVault := t.TempDir()
	res2, err := hooks.Install(hooks.InstallConfig{ProjectDir: withVault, VaultPath: "/data/my-vault"})
	require.NoError(t, err)
	assert.Contains(t, res2.SettingsStanza, "VAULTMIND_VAULT=",
		"--vault must bake VAULTMIND_VAULT into the stanza")
	assert.Contains(t, res2.SettingsStanza, "/data/my-vault")
}
