// Package testvault provides shared, per-process test fixtures built from the
// committed fixture vault. It builds the vault index once per test binary and hands
...
// FixtureVault returns the path to the committed test fixture vault
// (test/fixtures/testvault), resolved relative to THIS source file so tests
// never hardcode a fragile relative path or depend on a personal vault. The
// fixture is a small, generic cognitive-science vault sized to satisfy the
// graph/resolve/index tests (it carries the same note ids they assert).
func FixtureVault() string {
	_, thisFile, _, _ := runtime.Caller(0)
	return filepath.Clean(filepath.Join(filepath.Dir(thisFile), "..", "..", "test", "fixtures", "testvault"))
}

// IndexedFixtureVault copies the fixture vault into a fresh temp dir, builds its
// index there, and returns the temp vault path — for tests that need a vault
// directory with a real .vaultmind/index.db (e.g. cmdutil.OpenVaultDB), rather
// than the raw shared DB from OpenSharedDB.
func IndexedFixtureVault(tb testing.TB) string {
	tb.Helper()
	dst := tb.TempDir()
	if err := os.CopyFS(dst, os.DirFS(FixtureVault())); err != nil {
		tb.Fatalf("testvault: copy fixture vault: %v", err)
	}
	cfg, err := vault.LoadConfig(dst)
	if err != nil {
		tb.Fatalf("testvault: load fixture config: %v", err)
	}
	idxr := index.NewIndexer(dst, filepath.Join(dst, ".vaultmind", "index.db"), cfg)
	if _, err := idxr.Rebuild(); err != nil {
		tb.Fatalf("testvault: index fixture vault: %v", err)
	}
	return dst
}
...
// fixture vault, so the single-path lock is the honest contract.
//
