...
func TestIndex_FullRebuildHumanOutputShapeDiffersFromIncremental(t *testing.T) {
	...
	// Full rebuild reports its purge count too (issue #40): a rebuild that
	// drops newly-excluded / deleted notes must surface "deleted" so the
	// operator can trust the rebuilt index reflects the current include set.
