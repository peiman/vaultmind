...
func TestCallerLine_ComposesUserAtHost(t *testing.T) {
	got := callerLine("hook", map[string]any{"user": "user", "host": "box"})
	assert.Contains(t, got, "caller=hook")
	assert.Contains(t, got, "operator=user@box")
	assert.NotContains(t, got, "operator=user ")
...
func TestFormatNoteTrace_CarriesCallerAttribution(t *testing.T) {
	...
	assert.Contains(t, out, "caller=persona-hook")
