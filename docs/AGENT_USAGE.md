...
**Getting the binary.** Three install paths, each yielding a retrieval tier: (a) `go install github.com/peiman/vaultmind@latest` → pure-Go **MiniLM**, every platform, no deps; (b) the **prebuilt ORT archive** (`vaultmind_<version>_<os>_<arch>_ort.tar.gz` from the [release](https://github.com/peiman/vaultmind/releases), extract, run) → full **BGE-M3** 4-way hybrid on darwin-arm64/linux-amd64, no build; (c) from source (`git clone` + `task setup:ort` + `task build`) → full hybrid, any platform. To **update** an existing install, repeat your path (`@latest` / re-download the archive / `git pull && task build`). When replacing a binary by hand, extract elsewhere and **`mv` it into place** rather than copying over the file you are replacing: `mv` renames atomically onto a fresh inode, so there is no window where the binary is half-written. Copying onto the existing file has been observed once on macOS to fail with a codesigning SIGKILL (exit 137, `OS_REASON_CODESIGNING`) during a live upgrade, leaving the *new* binary unable to run at all; `xattr -cr` did not help and replacing via a fresh inode did. **The cause is not established** — neither the reporter nor we could reproduce it in isolation with two real builds, so some condition of a long-running install is involved that a fresh test copy does not have. The `mv` form is recommended because it is correct regardless: atomic, fresh inode, no half-written window. Confirm with `vaultmind version`; `vaultmind doctor` reports your retrieval tier and warns if degraded. `vaultmind doctor` also asks the Go module proxy once a day whether a newer release exists and prints a line if so — it sends nothing about you or your vault; `VAULTMIND_NO_UPDATE_CHECK=1` opts out.
...
<!-- checksum:c23325e2c0ff1b76fef732677b9bd2fea2a3a2ba74e299f305f9d9a602e06929 -->
# VaultMind Commands
