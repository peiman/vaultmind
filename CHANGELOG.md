...
- **A note is embedded as soon as it is written.** A changed note loses its
  ...
  uses. A vault never embedded is left alone; BGE-M3 on a build without the
...
> `vaultmind hooks install <project> --merge --force` (a knowledge project's
> remembered profile is kept). Until you do, the reach hook in your project
> keeps approving the commands it annotates.
